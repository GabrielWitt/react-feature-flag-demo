...
import type { DashboardPayment, DashboardReservation } from '../../services/usersApi';
...
const FALLBACK_PAYMENTS: DashboardPayment[] = [
  {
    id: 1,
    date: 'Sep 05, 2025',
    description: 'Monthly Lease - September',
    unit: 'B402',
    amount: '$1,200.00',
  },
...
const ClientHome = () => {
  ...
  const upcomingReservations =
    tenant?.tenantDashboard?.upcomingReservations ?? FALLBACK_RESERVATIONS;

  const reservationStatusLabel = useMemo(
    () => (tenant?.status === 'inactive' ? 'Review Required' : 'Confirmed'),
    [tenant?.status],
  );
  ...
  return (
    <MainLayout
      title="My Apartment"
      subtitle={`Welcome back, ${tenant?.name ?? user?.name ?? 'Tenant'}`}
    >
